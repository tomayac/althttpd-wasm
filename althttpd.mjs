...
["_main","__emscripten_thread_init","__emscripten_thread_exit","__emscripten_thread_crashed","__emscripten_thread_mailbox_await","__emscripten_tls_init","_pthread_self","checkMailbox","establishStackSpace","invokeEntryPoint","PThread","_fflush","__emscripten_proxy_main","__emscripten_check_mailbox","onRuntimeInitialized"].forEach((prop) => {
...
var wasmBinaryFile;
if (Module['locateFile']) {
  wasmBinaryFile = 'althttpd.wasm';
  if (!isDataURI(wasmBinaryFile)) {
    wasmBinaryFile = locateFile(wasmBinaryFile);
  }
} else {
  // Use bundler-friendly `new URL(..., import.meta.url)` pattern; works in browsers too.
  wasmBinaryFile = new URL('althttpd.wasm', import.meta.url).href;
}
...
// end include: preamble.js
  ...
  var PThread = {unusedWorkers:[],runningWorkers:[],tlsInitFunctions:[],pthreads:{},nextWorkerID:1,debugInit:function() {
        ...
        if (!Module['locateFile']) {
          worker = new Worker(new URL('althttpd.worker.js', import.meta.url));
        } else {
        // Allow HTML module to configure the location where the 'worker.js' file will be loaded from,
        // via Module.locateFile() function. If not specified, then the default URL 'worker.js' relative
        // to the main html file is loaded.
        var pthreadMainJs = locateFile('althttpd.worker.js');
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(7, 1, fd, suggestFD, flags);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(18, 1, path, buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(20, 1, which, timeout_ms);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(21, 1, socketId, binaryData, dataLength);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(22, 1, __environ, environ_buf);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(23, 1, penviron_count, penviron_buf_size);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(24, 1, fd);
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(27, 1, fd, iov, iovcnt, pnum);
...
var proxiedFunctionTable = [null,_proc_exit,exitOnMainThread,pthreadCreateProxied,___syscall__newselect,___syscall_chdir,___syscall_dup,___syscall_dup3,___syscall_faccessat,___syscall_fcntl64,___syscall_fstat64,___syscall_getcwd,___syscall_getdents64,___syscall_ioctl,___syscall_lstat64,___syscall_newfstatat,___syscall_openat,___syscall_pipe,___syscall_stat64,___syscall_unlinkat,__setitimer_js,_emscripten_websocket_send_binary,_environ_get,_environ_sizes_get,_fd_close,_fd_read,_fd_seek,_fd_write];
